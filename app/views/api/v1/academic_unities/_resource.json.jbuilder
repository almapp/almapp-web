json.extract! resource, :id, :abbreviation, :short_name, :name, :faculty_id, :address, :url, :email, :facebook, :twitter, :information

json.localization do
  if resource.localization.present?
    json.cache! ['compact', resource.localization], expires_in: very_long do
      json.partial! template_for_resource(resource.localization, 'compact'), resource: resource.localization
    end
  else
    json.null!
  end
end

json.extract! resource, :created_at, :updated_at